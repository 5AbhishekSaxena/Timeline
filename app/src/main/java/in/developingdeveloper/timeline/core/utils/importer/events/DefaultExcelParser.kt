...
class DefaultExcelParser @Inject constructor(
    @ApplicationContext
    private val context: Context,
) : ExcelParser {

    override suspend fun parse(fileUri: String): List<ParsedEvent> {
        return parse(fileUri.toUri())
    }

    private suspend fun parse(fileUri: Uri) = withContext(Dispatchers.IO) {
